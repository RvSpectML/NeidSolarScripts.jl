...
line_width_50_default = 7.9e3
 ...
 if isfile(joinpath("data","solar_line_list_espresso.csv"))  && false
...
good_orders_to_try = map(ord->[ord],56:111)
for good_orders in good_orders_to_try
       outputs_filename = joinpath(output_dir,"solar_" * date_str * "_ord=" * string(first(good_orders)) * ".jld2")
#good_orders = 56:111 # orders_to_use_default(NEID2D())
 ...
 outputs["order_snrs"*"_ord=" *string(first(good_orders))] = order_snrs
 outputs["order_sum_flux"*"_ord=" *string(first(good_orders))] = order_sum_flux
 outputs["order_mean_flux"*"_ord=" *string(first(good_orders))] = order_mean_flux
 outputs["order_weights"*"_ord=" *string(first(good_orders))] = order_weights
...
#msf = 1.24; fwtf = 1.5   # two pixels
 ...
 ((ccfs_espresso, ccf_vars_espresso), v_grid) = ccf_total(order_list_timeseries, line_list_espresso, pipeline_plan,
  ...
  outputs["ccfs_espresso"*"_ord=" *string(first(good_orders))] = ccfs_espresso
  outputs["ccf_vars_espresso"*"_ord=" *string(first(good_orders))] = ccf_vars_espresso
 ...
 outputs["rvs_ccf_espresso_ord=" *string(first(good_orders))] = rvs_ccf_espresso
 outputs["σ_rvs_espresso_ord=" *string(first(good_orders))] = σ_rvs_espresso
 if any(isnan.(rvs_ccf_espresso))
    continue
 end
 ...
 robust_one_sigma = (quantile(rvs_ccf_espresso[idx_to_fit],0.841)-quantile(rvs_ccf_espresso[idx_to_fit],0.159))/2
 idx_for_robust_fit = idx_to_fit[mean_rv_to_fit-3*robust_one_sigma .<= rvs_ccf_espresso[idx_to_fit] .<= mean_rv_to_fit+3*robust_one_sigma]
 ...
 outputs["times_binned"*"_ord=" *string(first(good_orders))] = times_binned
 outputs["rvs_binned"*"_ord=" *string(first(good_orders))] = rvs_binned
 outputs["rv_rms"*"_ord=" *string(first(good_orders))] = std(rvs_ccf_espresso[idx_to_fit].-v_fit[idx_to_fit])
 outputs["rv_0"*"_ord=" *string(first(good_orders))] = v_t0
 outputs["rv_mean"*"_ord=" *string(first(good_orders))] = mean(rvs_binned[idx_to_fit_binned])
 outputs["rv_slope"*"_ord=" *string(first(good_orders))] = linfit[1]/24
 outputs["rv_rms_binned"*"_ord=" *string(first(good_orders))] = std(rvs_binned[idx_to_fit_binned].-v_fit_binned[idx_to_fit_binned])
 ...
 xlims!(plt,-3.5,3.5)
 ylims!(plt,minimum(rvs_ccf_espresso.-v_fit)-0.5*robust_one_sigma, maximum(rvs_ccf_espresso.-v_fit)+0.5*robust_one_sigma)
 xlabel!(plt,"Time (hr)")
 ylabel!(plt,"RV - linear fit (m/s)")
 title!(plt,"Sun " * date_str * " CCF ESPRESSO " * string(first(good_orders)) * "\n" *
 ...
 display(plt)
 savefig(joinpath(output_dir,"rvs_" * date_str * "_ord=" * string(first(good_orders)) * " - " * string(last(good_orders)) * ".png"))
 ...
 title!(plt,"Sun " * date_str * ": " * string(first(good_orders)) * "\nCCF - <CCF>, ESPRESSO mask + SNR weights")
 ...
 display(plt)
 savefig(joinpath(output_dir,"ccf_heatmaps_sun_" * date_str * "_ord=" * string(first(good_orders)) * " - " * string(last(good_orders)) * "_espresso_msf=" * string(round(msf,digits=1)) * ".png"))

 outputs["mean_ccf" * "_ord=" * string(first(good_orders))] = ccf_template
...
#plt = heatmap(view(v_grid,cols_to_fit),(times[1:size(ccfs_norm,2)]).*24,ccf_resid_minus_rv_proj[cols_to_fit,:]')
 ...
 ylims!(plt,-3.5,3.5)
 display(plt)
 savefig(joinpath(output_dir,"ccf_resid_heatmaps_sun_" * date_str * "_ord=" * string(first(good_orders)) * " - " * string(last(good_orders)) * "_espresso_msf=" * string(round(msf,digits=1)) * ".png"))

 save(outputs_filename, outputs)

end # for loop over good_orders
