...
if isfile(joinpath(output_dir,"solar_" * date_str * ".jld2"))
  exit()
end
...
msf = 3.5; fwtf = 1.5
 ...
 ((ccfs_espresso, ccf_vars_espresso), v_grid) = ccf_total(order_list_timeseries, line_list_espresso, pipeline_plan,
   #mask_type=:gaussian,
   mask_scale_factor=msf, range_no_mask_change=5*line_width_50, ccf_mid_velocity=ccf_mid_velocity, v_step=100,
 ...
 idx_first_good_obs_of_day = findfirst(t->t-first(times)>delay_until_first_obs_used_for_fit,times)
 alt_sun = map(i->calc_solar_alt(all_spectra[i].metadata[:bjd]),1:length(all_spectra))
 time_max_alt_sun = times[argmax(alt_sun)]
 outputs["times"] = times .- time_max_alt_sun
 outputs["rvs_ccf_espresso"] = rvs_ccf_espresso
 outputs["σ_rvs_espresso"] = σ_rvs_espresso


 idx_last_obs_to_fit = findlast(t->t-time_max_alt_sun<= time_max_alt_sun - times[idx_first_good_obs_of_day],times)
 idx_to_fit = idx_first_good_obs_of_day:idx_last_obs_to_fit
...
using Plots
 ...
 plt = scatter( (times.-time_max_alt_sun).*24, rvs_ccf_espresso.-v_fit,  ms=1.5, label=:none)
 scatter!(plt,(times_binned.-time_max_alt_sun).*24, rvs_binned.-v_fit_binned, ms=4.0, label=:none)
 if first(idx_to_fit) > 1
   vline!(plt,[(times[first(idx_to_fit)-1]+times[first(idx_to_fit)])/2-time_max_alt_sun].* 24, label=:none, color=:grey)
   if last(idx_to_fit) < length(times)
 end
    vline!(plt,[(times[last(idx_to_fit)]+times[last(idx_to_fit)+1])/2-time_max_alt_sun].* 24, label=:none, color=:grey)
 end
 vline!(plt,[time_max_alt_sun-time_max_alt_sun].* 24, label=:none, color=:orange)
 ...
 ccfs_norm = EchelleCCFs.calc_normalized_ccfs( v_grid, ccfs_espresso, v_mid=ccf_mid_velocity, dv_min=2.5*line_width_50,dv_max= 2*max_bc )
 ...
 plt = heatmap(view(v_grid,cols_to_fit),(times[1:size(ccfs_norm,2)].-time_max_alt_sun).*24,view(ccfs_norm,cols_to_fit,:)'.-view(ccf_template,cols_to_fit,:)')
 xlabel!(plt,"v (m/s)")
 ...
 xlims!(plt,ccf_mid_velocity-2*line_width_50,ccf_mid_velocity+2*line_width_50)
 hline!(plt,[times[first(idx_to_fit)]-time_max_alt_sun, times[last(idx_to_fit)]-time_max_alt_sun].*24, label=:none, color=:grey)
...
(order_ccfs, order_ccf_vars, v_grid_order_ccfs) = ccf_orders(order_list_timeseries, line_list_espresso, pipeline_plan, mask_scale_factor=msf, ccf_mid_velocity=ccf_mid_velocity,
 v_step=200, v_max=max(5*line_width_50,2*max_bc), orders_to_use=good_orders, allow_nans=true, calc_ccf_var=true,recalc=true)
 outputs["order_ccfs"] = order_ccfs
 outputs["order_ccf_vars"] = order_ccf_vars
 outputs["v_grid_order_ccfs"] =v_grid_order_ccfs

obs_id = 1
heatmap(v_grid_order_ccfs, 1:size(order_ccfs,2), (order_ccfs[:,:,obs_id]./mean(order_ccfs[:,:,obs_id],dims=1))')
xlabel!("v (m/s)")
ylabel!("Order (ignore #)")
title!("Sun " * date_str  )
#xlims!(ccf_mid_velocity-RvSpectMLBase.max_bc,ccf_mid_velocity+RvSpectMLBase.max_bc)
xlims!(ccf_mid_velocity-2.5*line_width_50,ccf_mid_velocity+2.5*line_width_50)
...
using JLD2, FileIO
save(joinpath(output_dir,"solar_" * date_str * ".jld2"), outputs)
