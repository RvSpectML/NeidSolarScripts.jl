...
begin
	ccf_path = "/mnt/data_simons/NEID/v0.7develop20210501/outputs2"
...
begin  # Parameters for picking which days to calculate RVs for
	...
	solar_hour_angle_threshold = 1.5
end;
...
$(@bind order_daily_rms_lo Slider(0.0:10; default=0))  
$(@bind order_daily_rms_hi Slider(0.0:50.0; default=30.0))

"""

# ╔═╡ fe0b7b47-39bb-45f8-873e-9a4a07d7d1b3
md"""
Order RV slope limits (lo, hi) $(@bind order_slope_lo Slider(-500.0:0.0; default=-50))  $(@bind order_slope_hi Slider(0.0:500.0; default=50.0))
...
31 $(@bind use_ord_31 CheckBox(default=true))
...
41 $(@bind use_ord_41 CheckBox(default=true))
...
51 $(@bind use_ord_51 CheckBox(default=true))
...
order_idx_for_rvs_manual = vcat(1,4:6,8:10,12,18:20,22)
#order_idx_for_rvs_manual = vcat(1,4:12,14:22,24:25)
#order_idx_for_rvs_manual = vcat(14,:21)
#order_idx_for_rvs_manual = vcat(24:29)
#order_idx_for_rvs_manual = vcat(36:40)
#order_idx_for_rvs_manual = vcat(46:50)
...
md"""
Order RV RMS limits (lo, hi) 
$(@bind order_rv_rms_lo Slider(0.0:2.0; default=0))  
$(@bind order_rv_rms_hi Slider(0.5:200.0; default=100))
"""
...
# ╔═╡ ac1b2ad6-ad7b-4487-8da3-336f4a171852
md"""
## Orders RVs
...
begin
	daily_rms_across_orders = map(obsid->NaNMath.std((day_orders.order_rv_sha0[obsid].-median_order_rv)[order_idx_for_rvs]),1:size(day_orders,1))
	mask_decent_days = daily_rms_across_orders.<10
end;

# ╔═╡ 9dc77012-f845-4a5b-a059-fd13c1308b3d
begin
	histogram(daily_rms_across_orders[mask_decent_days],nbins=40,label=:none)
	xlabel!("RMS (m/s)")
	ylabel!("Count")
...
begin
	...
	local days_to_plt = plt_all_days ? (1:size(day_orders,1)) : (1:size(day_orders,1))[mask_decent_days]
	local pal = palette(:berlin, length(orders_to_plt))
	for (i,ordidx) in enumerate(orders_to_plt)
		scatter!(plt,Date.(Dates.julian2datetime.(day_orders.t_sha0)),	map(obsidx->day_orders[obsidx,:rms_order_rv][ordidx],(1:size(day_orders,1))[days_to_plt]),label=:none,c=pal[i])
	end
	xlabel!(plt,"Time (d)")
...
begin
	...
	for (i,ord) in enumerate(order_idx_for_rvs)	
		histogram!(plt,day_orders[ord,:rms_order_rv_binned],bin=range(0,stop=order_rv_rms_hi,length=40), color=pal[i],label=string(ord))
	end
	xlabel!("RMS (m/s)")
	ylabel!("Count")
...
function summarize_day_total(day_to_plt::Integer; df::DataFrame, orders, weights)
	...
	if sum(mask) >= 1
		...
		(t_binned, rvs_binned) = #, num_obs_in_bin) = 
			bin_times_and_rvs_max_Δt(times=times[mask],rvs=rv_tot[mask],Δt_threshold= 5/(24*60))
		rms_rv_binned = std(rvs_binned)
		daily_fit = Polynomials.fit(times[mask].-tsha0,rv_tot[mask],1)
		rms_rv_binned_fit = std(rvs_binned .-daily_fit.(t_binned.-tsha0))
...
$(@bind daily_rv_rms_hi Slider(0.0:0.2:10.0; default=default_daily_rv_rms_hi))
"""
...
begin
	days_to_plt = plt_all_days ? (1:size(day_orders,1)) : (1:size(day_orders,1))[mask_decent_days .& .!isnan.(day_comb.rv_sha0) ]#.& .!(mapreduce(s->datafiles_use.datestr .==s,.|,[ "20210201", "20210202"]))] 
	Δrv = day_comb.rv_sha0.-NaNMath.mean(day_comb.rv_sha0[days_to_plt])	
end

# ╔═╡ d4e7987c-94bf-4194-8e16-88c2581832d1
begin
	local days_to_plt = plt_all_days ? (1:size(day_orders,1)) : (1:size(day_orders,1))[mask_decent_days .& .!isnan.(day_comb.rv_sha0)] #.& .!(mapreduce(s->datafiles_use.datestr .==s,.|,[ "20210201", "20210202"]))] 
...
$(@bind daily_slope_lo Slider(-10.0:0.0; default=minimum(day_comb.rv_slope)))
$(@bind daily_slope_hi Slider(0.0:10.0; default=maximum(day_comb.rv_slope)))
...
begin
	plot(legend=:none)
	local days_to_plt = plt_all_days ? (1:size(day_orders,1)) : (1:size(day_orders,1))[mask_decent_days]
	histogram(Date.(Dates.julian2datetime.(day_comb.t_sha0[days_to_plt])),day_comb.rv_slope[days_to_plt],bins=range(daily_slope_lo,stop=daily_slope_hi,length=30),label=:none)
